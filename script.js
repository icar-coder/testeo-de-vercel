document.addEventListener('DOMContentLoaded', function() {
    ...
    function procesarTodasLasHojas() {
        if (!workbookGlobal) return;
        const filterEmptyVal = filterEmpty ? filterEmpty.checked : true;
        const headerRowNum = headerRow ? parseInt(headerRow.value) - 1 : 0;
    ...
    function actualizarStats(productos) {
        ...
        if (totalMarcas) totalMarcas.textContent = marcas.size;
        if (precioPromedio) precioPromedio.textContent = `$${avg.toFixed(2)}`;
    }
